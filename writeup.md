...
* This project came with "challenge_video.mp4". The challenging aspect of this video is that this video has many lines on the pavement that are parallel with the lane lines. Because of this additional line features on the pavement, even the initial fitted line converged to the wrong lines. Since the initial fitted line was wrong, all the lane lines identified by the alrogithm in the subsequent frame were wrong. The possible improvement to fix this problem is to add sanity check to the fitted lines. If the fitted lines do not seem valid, pipeline should not use the fitted line.

* This project came with "harder_challenge_video.mp4". The features found in this video that make the lane line finding very difficult are; downhill; reflected dashboard in the image; a lot of tree shadow on the road; winding road; white out because of the light reflected on the road; lane lines out of field of view because of the sharp turn. To address these numerous issues, the pipeline should be much smarter and fault-tolerent.